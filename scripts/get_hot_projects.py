#!/usr/bin/env python3
"""
Hot projects fetcher

Env vars:
- GITHUB_TOKEN (required)
- TOP_N (default 20)             -> final number of repos in the report
- DAYS (default 7)               -> lookback window (pushed:>DATE)
- KEYWORDS (optional)            -> comma-separated keywords/phrases, e.g. "AI agent,harness engineering"
- ORGS (optional)                -> comma-separated org names, e.g. "harness,openai"
- EXTRA_REPOS (optional)         -> comma-separated owner/repo entries to always include
- EXTRA_REPOS_URL (optional)     -> URL to plaintext list of owner/repo (one per line)
- PER_SOURCE (default 50)        -> how many results to fetch per keyword/org source (will be capped by API)
- SORT (default "stars")         -> "stars" or "pushed" (final sort order)
- VERBOSE (optional)             -> if set, print more debug logs

Outputs:
- results/hot-projects-YYYY-MM-DD.md
- results/hot-projects-YYYY-MM-DD.json
"""
import os
import sys
import requests
...
SORT = os.getenv("SORT", "stars").lower()  # stars or pushed
VERBOSE = bool(os.getenv("VERBOSE"))

API_SLEEP_ON_ERROR = 2  # seconds to backoff on transient errors
...
def search_repos(query, max_results):
    """Search repos with pagination; returns list of repo objects (as returned by API)."""
    results = []
...
def get_repo(full_name):
    ...
    try:
        resp = requests.get(f"{GITHUB_API}/repos/{full_name}", headers=headers, timeout=20)
        if resp.status_code == 200:
            return resp.json()
        else:
...
def collect_candidates():
    ...
    for kw in KEYWORDS:
        ...
        items = search_repos(query, PER_SOURCE)
        for r in items:
